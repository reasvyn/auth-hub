...
export function createNextJsAdapter(options: NextJsAdapterOptions = {}): AuthAdapter {
  ...
  return {
    login: (credentials) =>
      apiFetch<AuthResponse>(`${base}/login`, {
        method: 'POST',
        body: JSON.stringify(credentials),
      }),

    register: (credentials) =>
      apiFetch<AuthResponse>(`${base}/register`, {
        method: 'POST',
        body: JSON.stringify(credentials),
      }),
    ...
    refreshToken: (token) =>
      apiFetch<AuthResponse>(`${base}/refresh`, {
        method: 'POST',
    ...
    verifyMagicLink: (token) =>
      apiFetch<AuthResponse>(`${base}/magic-link/verify`, {
        method: 'POST',
    ...
    confirmPasswordReset: (token, newPassword) =>
      apiFetch<void>(`${base}/password/reset/confirm`, {
        method: 'POST',
        body: JSON.stringify({ token, newPassword }),
      }),

    changePassword: (currentPassword, newPassword) =>
      apiFetch<void>(`${base}/password/change`, {
        method: 'POST',
    ...
    setupMFA: (method: TwoFactorMethod) =>
      apiFetch<MFASetupData>(`${base}/mfa/setup`, {
        method: 'POST',
        body: JSON.stringify({ method }),
      }),

    verifyMFA: (code: string, method: TwoFactorMethod) =>
      apiFetch<AuthResponse>(`${base}/mfa/verify`, {
        method: 'POST',
        body: JSON.stringify({ code, method }),
      }),

    disableMFA: (method: TwoFactorMethod, code: string) =>
      apiFetch<void>(`${base}/mfa/disable`, {
        method: 'POST',
    ...
    revokeAllSessions: () => apiFetch<void>(`${base}/sessions`, { method: 'DELETE' }),
  };
}
