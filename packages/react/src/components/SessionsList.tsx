...
export function SessionsList({ adapter, onRevoke, onRevokeAll, className }: SessionsListComponentProps) {
  const { sessions, loading, error, revoke, revokeAll } = useSession(adapter);
  ...
  return (
    <Card className={className}>
      <div className="flex items-center justify-between mb-4">
        ...
        {sessions.length > 1 && (
          <Button variant="secondary" onClick={handleRevokeAll} className="!w-auto text-xs px-3 py-1.5">
            Revoke all others
          </Button>
        )}
      </div>
            ...
            >
              <div className="flex flex-col gap-1 min-w-0">
                ...
                {session.metadata?.ipAddress && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">{session.metadata.ipAddress}</p>
                )}
